...
std::string T3_encode_simple_graph_with_constants_sample() {
    ...
    const std::string conv2dRescaleGraph =
        R""(
...
%uint_1935896203 = OpConstant %uint 1935896203
   ...
   %uchar_26 = OpConstant %uchar 26
    ...
    std::vector<uint32_t> spirvCode = sampleutils::spirv_assemble(conv2dRescaleGraph);
    ...
    vgflib::ModuleRef graphRef =
        encoder->AddModule(vgflib::ModuleType::GRAPH, // Explicitly call out this specific module type as Graph. This
                           ...
                           spirvCode // The SPIR-V binary code for the module
        );

    // Define the input resource
    std::vector<int64_t> inputShape = {1, 16, 16, 16};
    ...
    // Weights
    std::vector<int64_t> weightShape = {16, 2, 2, 16};
    vgflib::ResourceRef weightResRef = encoder->AddConstantResource( // this resource is going to be a graph constant
        vgflib::ToFormatType(VK_FORMAT_R8_SINT),                     // the weight elements will be int8 type
        weightShape,                                                 // 16x2x2x16 shape of weights
        {});
    std::vector<int8_t> weightData(16ULL * 2 * 2 * 16, 0); // This holds the actual bias values
    vgflib::ConstantRef weightConstRef =
        encoder->AddConstant(weightResRef, weightData.data(), sampleutils::sizeof_vector(weightData));
    ...
    encoder->AddModelSequenceInputsOutputs(
        {inputBindingRef},  // the input bindings of the whole model. Should all be of type ResourceCategory::INPUT
        {"input_0"},        // name of the input so that consumers can bind the input data correctly at runtime.
        {outputBindingRef}, // the output binding of the whole model. Should all be of type ResourceCategory::OUTPUT
    ...
    std::filesystem::path fullPath = std::filesystem::temp_directory_path();
    fullPath /= filename;

    std::ofstream vgfFile(fullPath.c_str(), std::ofstream::binary | std::ofstream::trunc);
    encoder->WriteTo(vgfFile);
    vgfFile.close();
