...
std::string T1_encode_simple_graph_sample() {
    ...
    const std::string singleMaxpoolGraph =
        R""(
...
%_arr_uint_uint_1 = OpTypeArray %uint %uint_1
    ...
    std::vector<uint32_t> spirvCode = sampleutils::spirv_assemble(singleMaxpoolGraph);
    ...
    encoder->AddModelSequenceInputsOutputs(
        {inputBindingRef},  // the input bindings of the whole model. Should all be of type INPUT
        {"input_0"},        // name of the input so that consumers can bind the input data correctly at runtime.
        {outputBindingRef}, // the output binding of the whole model. Should all be of type OUTPUT
